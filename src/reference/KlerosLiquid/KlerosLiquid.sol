...
contract RNG {
    ...
    function getUncorrelatedRN(uint _block) public returns (uint RN) {
        ...
        else return uint(keccak256(msg.sender, baseRN));
    }
}
...
contract KlerosLiquid is TokenController, Arbitrator {
    ...
    function castCommit(
        ...
        for (uint i = 0; i < _voteIDs.length; i++) {
            require(
                dispute.votes[dispute.votes.length - 1][_voteIDs[i]].account ==
                    msg.sender,
    ...
    function createDispute(
        ...
        returns (uint disputeID)
    {
        (uint96 subcourtID, uint minJurors) = extraDataToSubcourtIDAndMinJurors(
            _extraData
        );
    ...
    function appealCost(
        ...
        uint lastNumberOfJurors = dispute
            ...
            .length;
        if (
            lastNumberOfJurors >= courts[dispute.subcourtID].jurorsForCourtJump
        ) {
            // Jump to parent subcourt.
    ...
    function appealPeriod(
        ...
        if (dispute.period == Period.appeal) {
            start = dispute.lastPeriodChange;
                ...
                courts[dispute.subcourtID].timesPerPeriod[uint(Period.appeal)];
        } else {
            start = 0;
            end = 0;
        }
